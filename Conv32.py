...
summary = TensorboardSummary('./repr_dir/test6')
...
def prepare_data():
    ...
    trainset = datasets.CIFAR10('./data/', train=True, download=False, transform=transform_train)
...
def train(epoch, training_type):
    ...
    tb_writer.add_scalar('net1 train acc',acc1,epoch)
    tb_writer.add_scalar('net2 train acc',acc2,epoch)
...
def test():
    ...
    log(log_file, "\n")
    tb_writer.add_scalar('net1 val acc', acc1, epoch)
    tb_writer.add_scalar('net2 val acc', acc2, epoch)
...
def cal_dropped_filters(p):
    ...
    return dropped_filters, dropped_filters_weight
...
def re_initiate_zyy():
    ...
    for layer_index, dropped_filter_list in dropped_filters.items():
        ...
        if len(undropped_filter_list) != 0:
            q, r = linalg.qr(W.T, mode='economic')
            M, N = q.shape
...
while epoch < 109:
    ...
    dropped_filters, dropped_filters_weight = cal_dropped_filters(p)
    drop()
